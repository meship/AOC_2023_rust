...
fn create_count_map(input_str: &str) -> (HashMap<char, u16>, u16) {
    let mut count_map: HashMap<char, u16> = HashMap::new();
    let mut joker_count: u16 = 0;
    ...
    (count_map, joker_count)
}

 

fn hand_to_calss(hand: &str) -> u16{
    let (count_map, joker_count) = create_count_map(hand);
    if joker_count == 5{
        return 7;
    }
    ...
    let val: u16 = match vals[0]{
        5 => 7,
        4 => if joker_count != 0 {7} else {6},
        3 => {if joker_count != 0 {5 + joker_count} else if vals[1] == 2 {5} else {4}},
        2 => {if joker_count == 3 {7} 
              else if joker_count == 2 {6} 
              else if vals[1] == 2 {if joker_count == 1 {5} else {3}} 
              else {if joker_count ==1 {4} else{2} }},
        1 => {if joker_count == 4 {7}
             else if joker_count == 3 
             {6} else if joker_count == 2 {4} 
             else if joker_count == 1 {2} 
             else {1} }, 
        _ => 0,
